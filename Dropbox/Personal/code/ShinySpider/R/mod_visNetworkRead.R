...
mod_visNetworkRead_ui <- function(id){
  ns <- NS(id)
  tagList(
    inputPanel(
    ...
    visNetwork::visNetworkOutput(ns("visnetworkGraph"))
  )
}

#' visNetworkRead Server Functions
#'
#' @noRd
mod_visNetworkRead_server <- function(id, graph) {
  moduleServer(id, function(input, output, session){
    ns <- session$ns
    output$visnetworkGraph <- visNetwork::renderVisNetwork({
      g <- graph()
    ...
    observe({
      if(selectNode() != "ALL") {
        visNetwork::visNetworkProxy(ns("visnetworkGraph")) |>
